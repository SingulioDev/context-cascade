...
name: skill-forge
version: 2.2.0
description: Advanced skill creation system for Claude Code using 8-phase methodology (v2.0 adds Phase 0 expertise loading). Combines deep intent analysis, evidence-based prompting principles, and systematic skill engineering. Use when creating new skills or refining existing skills. Integrates with recursive improvement loop - can rebuild itself using previous version. Transforms skill creation from template filling into strategic design.
---
...
**Rationale**: Skill Forge is a meta-cognitive skill creation framework that operates entirely through file operations (Read, Write, Edit) and doesn't require external services, sandboxed execution, or persistent state beyond the filesystem.
...
## The Skill Forge Process (v2.0)

Skill Forge follows an **eight-phase methodology** (v2.0 adds Phase 0 for expertise loading and recursive improvement integration). Each phase builds on insights from previous phases while remaining flexible enough to adapt to the unique requirements of different skill types.

### Phase 0: Expertise Loading [NEW - v2.0]

Before beginning skill creation, load domain expertise if available.

**Check for Domain Expertise**:
```bash
# Detect domain from skill purpose
DOMAIN=$(detect_domain_from_purpose)
...
if expertise_exists:
  actions:
    ...
    - Apply: Use expertise to inform skill design
  benefits:
    - Skip search thrash (known file locations)
    - Follow conventions (documented patterns)
    - Prevent bugs (known issues)
else:
  actions:
    - Flag: Discovery mode
    - Plan: Generate expertise after skill creation
```

**Validation Gate**:
- [ ] Checked for relevant domain expertise
- [ ] Loaded expertise if available
- [ ] Applied expertise context to planning
...
- **agent-creator**: To create auditor agents for the improvement loop
- **prompt-architect**: To optimize skill descriptions and instructions
- **prompt-forge**: Receives improvement proposals for skills
- **eval-harness**: Gates all skill changes
...
Skill Forge is part of the recursive self-improvement loop:
...
Skill Forge (META-SKILL)
    ...
    +--> Can rebuild ITSELF (using previous version)
    +--> Applies proposals from Prompt Forge
```
...
input_contract:
  required:
    - skill_purpose: string  # What the skill should do
    - use_cases: list  # 3-5 concrete examples
  optional:
    - expertise_file: path  # Pre-loaded domain expertise
    - existing_skill: path  # Skill to improve
    - constraints: list  # Specific requirements
    - improvement_proposal: object  # From prompt-forge

output_contract:
  required:
    - skill_file: path  # Created/updated SKILL.md
    - validation_report: object  # Phase 7 results
    - version: semver  # Skill version
  optional:
    - bundled_resources: list  # Scripts, references, assets
    - process_diagram: path  # GraphViz .dot file
    - expertise_delta: object  # Learnings for expertise update
```

### Eval Harness Integration

Created skills are tested against:

```yaml
benchmark: skill-generation-benchmark-v1
  tests:
    - sg-001: Micro-Skill Generation
    - sg-002: Complex Skill Generation
  minimum_scores:
    functionality: 0.75
    structure_compliance: 0.80
    safety_coverage: 0.75

regression: skill-forge-regression-v1
  tests:
    - sfr-001: 8-phase structure preserved (must_pass)
    - sfr-002: Contract specification present (must_pass)
    - sfr-003: Error handling included (must_pass)
    - sfr-004: Test cases generated (must_pass)
```

### Self-Rebuild Safety

When Skill Forge improves itself:

```yaml
self_rebuild_protocol:
  trigger: "Bootstrap loop improvement cycle"

  safeguards:
    - uses_previous_version: true  # NEVER use current to rebuild current
    - archive_before_apply: true
    - eval_harness_required: true
    - human_gate_for_breaking: true

  process:
    1. "Load skill-forge version N-1 from archive"
    2. "Apply improvement proposal from prompt-forge"
    3. "Generate skill-forge version N+1 candidate"
    4. "Test candidate against eval harness"
    5. "If passed: Commit. If failed: Reject."

  forbidden:
    - "Removing 8-phase structure"
...
```yaml
namespaces:
  - skill-forge/creations/{skill}: Skills created
  - skill-forge/improvements/{id}: Improvement cycles
  - skill-forge/self-rebuilds/{id}: Self-improvement cycles
  - skill-forge/metrics: Performance tracking
  - improvement/audits/skill-forge: Audits of this skill
  - improvement/commits/skill-forge: Version history
```

### Uncertainty Handling

When skill requirements are unclear:
...
```yaml
rollback:
  archive_location: ".claude/skills/{skill}/.archive/"
  naming: "SKILL-v{version}.md"
  retention: "90 days minimum"

  trigger_rollback:
    - command: "/rollback {skill} {version}"
    - automatic: "If regression detected in monitoring"

  process:
    1. "Verify archive exists"
    2. "Copy archived version to active location"
