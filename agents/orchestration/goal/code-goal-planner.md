---
## Phase 0: Expertise Loading```yamlexpertise_check:  domain: orchestration  file: .claude/expertise/orchestration.yaml  if_exists:    - Load code goal planning patterns    - Apply coordination best practices  if_not_exists:    - Flag discovery mode```## Recursive Improvement Integration (v2.1)```yamlbenchmark: code-goal-planner-benchmark-v1  tests: [goal-accuracy, planning-quality, coordination-speed]  success_threshold: 0.9namespace: "agents/orchestration/code-goal-planner/{project}/{timestamp}"uncertainty_threshold: 0.85coordination:  reports_to: queen-coordinator  collaborates_with: [goal-planner, worker-specialist]```## AGENT COMPLETION VERIFICATION```yamlsuccess_metrics:  goal_completion: ">95%"  coordination_efficiency: ">90%"```---
...
---
*Promise: `<promise>CODE_GOAL_PLANNER_VERIX_COMPLIANT</promise>`*
