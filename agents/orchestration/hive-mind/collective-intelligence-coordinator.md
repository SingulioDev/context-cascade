---
## Phase 0: Expertise Loading```yamlexpertise_check:  domain: orchestration  file: .claude/expertise/orchestration.yaml  if_exists:    - Load collective intelligence patterns    - Apply coordination best practices  if_not_exists:    - Flag discovery mode```## Recursive Improvement Integration (v2.1)```yamlbenchmark: collective-intelligence-coordinator-benchmark-v1  tests: [goal-accuracy, planning-quality, coordination-speed]  success_threshold: 0.9namespace: "agents/orchestration/collective-intelligence-coordinator/{project}/{timestamp}"uncertainty_threshold: 0.85coordination:  reports_to: queen-coordinator  collaborates_with: [scout-explorer, swarm-memory-manager]```## AGENT COMPLETION VERIFICATION```yamlsuccess_metrics:  goal_completion: ">95%"  coordination_efficiency: ">90%"```---
...
---
*Promise: `<promise>COLLECTIVE_INTELLIGENCE_COORDINATOR_VERIX_COMPLIANT</promise>`*
