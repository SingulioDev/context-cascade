...
### 1. Execution Validation Over Static Appearance
Theater code looks correct in static analysis but fails during runtime. True validation requires actual execution with realistic inputs.

**In practice**:
- Never trust code that "looks right" without running it
- Create isolated test environments to execute suspicious code safely
- Validate outputs match expected behavior, not just structure
- Test edge cases and error paths, not just happy paths

### 2. Systematic Pattern Recognition
Theater code follows predictable anti-patterns: empty catch blocks, no-op functions, trivial returns, always-passing tests.

**In practice**:
- Scan for empty catch blocks that swallow errors silently
- Detect functions that always return null, true, or empty objects
- Identify tests with tautological assertions (expect(true).toBe(true))
- Flag TODO/FIXME comments indicating incomplete work

### 3. Fix with Context Preservation
Automated fixes should address theater issues without breaking existing integrations or introducing new problems.

**In practice**:
- Create backups before applying automated fixes
- Run full test suite after each fix to detect regressions
- Apply fixes incrementally (one file at a time) to isolate issues
- Document why code was theater (missing requirements, placeholder logic)
...
|--------------|---------|----------|
| **Assuming Compilation = Correctness** | Code compiles and passes linting but doesn't implement actual business logic | Execute code with realistic inputs and validate outputs match requirements. Static analysis finds syntax errors, not theater logic. |
| **Trusting AI-Generated Code Blindly** | AI tools can generate structurally correct code that lacks meaningful implementation | Always validate AI-generated code through execution tests. Theater code often comes from AI completing stubs without real logic. |
| **Over-Reliance on Code Coverage** | 100% test coverage with always-passing tests provides false confidence | Inspect test quality, not just coverage percentage. Tests must assert actual behavior, not trivial conditions. |

---

## Conclusion

Theater code detection addresses a critical gap in software quality assurance: code that appears complete but delivers no functionality. Traditional quality checks focus on syntax, style, and static correctness, but theater code passes all these checks while failing the fundamental requirement of working correctly. This skill fills that gap by combining pattern recognition, implementation analysis, and execution validation to identify non-functional code before it reaches production.
