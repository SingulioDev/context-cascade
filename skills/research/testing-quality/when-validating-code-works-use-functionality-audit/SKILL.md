...
### 1. Execution Over Appearance
Code that compiles and passes linting may still be non-functional. True validation requires running code with realistic inputs in isolated environments.

**In practice**:
- Create sandboxed test environments to prevent side effects
- Execute code with realistic data, not just minimal examples
- Validate outputs match expected behavior, not just structure
- Test edge cases and error paths, not just happy paths

### 2. Systematic Debugging Methodology
When tests fail, use structured root cause analysis rather than trial-and-error fixes. Build error trees to identify actual problems, not symptoms.

**In practice**:
- Analyze stack traces to pinpoint failure location precisely
- Use debugger breakpoints and variable inspection, not console.log thrash
- Categorize failures by type (syntax, runtime, logic, integration)
- Fix root causes, not surface-level symptoms

### 3. Non-Regressive Fixes
Every fix should improve functionality without breaking existing behavior. Validate that fixes resolve issues AND preserve working code.

**In practice**:
- Run full test suite after every fix to detect regressions immediately
- Apply fixes incrementally (one issue at a time) to isolate impact
- Use version control snapshots to enable rollback if fixes cause problems
- Measure before/after coverage to ensure fixes improve, not reduce, testing
...
|--------------|---------|----------|
| **Assuming Tests Pass = Code Works** | Passing tests with poor coverage or weak assertions provide false confidence | Validate actual functionality through manual testing with realistic inputs. Check coverage gaps and strengthen assertions. |
| **Fixing Symptoms, Not Root Causes** | Quick patches that suppress errors without addressing underlying issues create technical debt | Use systematic debugging to identify root causes. Fix the actual problem, not just the error message. |
| **Testing in Production Environment** | Running untested code directly in production risks data corruption and service outages | Always use isolated sandbox environments for validation. Production should only run pre-validated, tested code. |

---

## Conclusion

Functionality auditing bridges the gap between code that exists and code that works. Static analysis tools catch syntax errors and style violations, but they cannot validate that code delivers its intended behavior when executed. This skill fills that gap by systematically creating test environments, executing code with realistic inputs, analyzing failures through structured debugging, and validating that fixes resolve issues without introducing regressions.
