...
# Enhancement Instructions for agent-creator
...
## Skill Purpose
--- name: agent-creator description: Creates specialized AI agents with optimized system prompts using the official 4-phase SOP methodology from Desktop .claude-flow, combined with evidence-based prom...
...
**Location**: `agent-creator/README.md`
...
**Location**: `agent-creator/examples/`
...
**Location**: `agent-creator/references/`
...
**Location**: `agent-creator/graphviz/`
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance agent-creator to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/agent-creator/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-agent-creator"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/agent-creator/status" --value "enhanced-to-Silver"
```
