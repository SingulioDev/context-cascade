...
# Enhancement Instructions for agentdb-optimization
...
## Skill Purpose
--- name: "AgentDB Performance Optimization" description: "Optimize AgentDB performance with quantization (4-32x memory reduction), HNSW indexing (150x faster search), caching, and batch operations. U...
...
**Location**: `agentdb-optimization/README.md`
...
**Location**: `agentdb-optimization/examples/`
...
**Location**: `agentdb-optimization/references/`
...
**Location**: `agentdb-optimization/graphviz/`
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance agentdb-optimization to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/agentdb-optimization/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-agentdb-optimization"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/agentdb-optimization/status" --value "enhanced-to-Silver"
```
