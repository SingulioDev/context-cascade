...
# Enhancement Instructions for agentdb-vector-search
...
## Skill Purpose
--- name: "AgentDB Vector Search" description: "Implement semantic vector search with AgentDB for intelligent document retrieval, similarity matching, and context-aware querying. Use when building RAG...
...
**Location**: `agentdb-vector-search/README.md`
...
**Location**: `agentdb-vector-search/examples/`
...
**Location**: `agentdb-vector-search/references/`
...
**Location**: `agentdb-vector-search/graphviz/`
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance agentdb-vector-search to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/agentdb-vector-search/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-agentdb-vector-search"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/agentdb-vector-search/status" --value "enhanced-to-Silver"
```
