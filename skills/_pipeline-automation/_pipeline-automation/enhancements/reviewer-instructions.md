...
# Enhancement Instructions for reviewer
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance reviewer to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/reviewer/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-reviewer"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/reviewer/status" --value "enhanced-to-Silver"
```
