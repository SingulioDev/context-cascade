...
# Enhancement Instructions for agentdb-memory-patterns
...
## Skill Purpose
--- name: "AgentDB Memory Patterns" description: "Implement persistent memory patterns for AI agents using AgentDB. Includes session memory, long-term storage, pattern learning, and context management...
...
**Location**: `agentdb-memory-patterns/README.md`
...
**Location**: `agentdb-memory-patterns/examples/`
...
**Location**: `agentdb-memory-patterns/references/`
...
**Location**: `agentdb-memory-patterns/graphviz/`
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance agentdb-memory-patterns to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/agentdb-memory-patterns/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-agentdb-memory-patterns"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/agentdb-memory-patterns/status" --value "enhanced-to-Silver"
```
