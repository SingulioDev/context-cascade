...
# Enhancement Instructions for researcher
...
**Location**: `researcher/references/`
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance researcher to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/researcher/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-researcher"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/researcher/status" --value "enhanced-to-Silver"
```
