...
# Enhancement Instructions for debugging
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance debugging to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/debugging/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-debugging"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/debugging/status" --value "enhanced-to-Silver"
```
