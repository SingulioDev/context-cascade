...
# Enhancement Instructions for tester
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance tester to Gold tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/tester/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-tester"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/tester/status" --value "enhanced-to-Gold"
```
