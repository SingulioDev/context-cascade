...
# Enhancement Instructions for agentdb-advanced
...
## Skill Purpose
--- name: "AgentDB Advanced Features" description: "Master advanced AgentDB features including QUIC synchronization, multi-database management, custom distance metrics, hybrid search, and distributed ...
...
**Location**: `agentdb-advanced/README.md`
...
**Location**: `agentdb-advanced/examples/`
...
**Location**: `agentdb-advanced/references/`
...
**Location**: `agentdb-advanced/graphviz/`
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance agentdb-advanced to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/agentdb-advanced/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-agentdb-advanced"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/agentdb-advanced/status" --value "enhanced-to-Silver"
```
