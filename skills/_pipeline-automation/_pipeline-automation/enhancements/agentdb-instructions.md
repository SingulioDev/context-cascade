...
# Enhancement Instructions for agentdb
...
## Skill Purpose
--- name: agentdb description: High-performance vector search and semantic memory for AI agents. Use when implementing RAG systems, semantic document retrieval, or persistent agent memory. Provides 15...
...
**BEFORE starting**:
```bash
npx claude-flow@alpha hooks pre-task --description "Enhance agentdb to Silver tier"
```

**DURING work**:
```bash
npx claude-flow@alpha hooks post-edit --file "{file}" --memory-key "skill-enhancement-pipeline/agentdb/{component}"
```

**AFTER completion**:
```bash
npx claude-flow@alpha hooks post-task --task-id "enhance-agentdb"
npx claude-flow@alpha memory store --key "skill-enhancement-pipeline/agentdb/status" --value "enhanced-to-Silver"
```
