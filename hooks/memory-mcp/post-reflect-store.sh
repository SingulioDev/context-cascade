#!/bin/bash
# Hook: Post-Reflect Store (Memory MCP) - Unix shell version
# Triggers after /reflect skill completes to store learnings
# Mirror of root post-reflect-store.sh for memory-mcp category
...
echo "[post-reflect] Storing learnings from: $LEARNINGS_FILE"

# Run the reflect_to_memory.py script
if python "$REFLECT_SCRIPT" --session-id "$SESSION_ID" --project "$PROJECT" --from-file "$LEARNINGS_FILE" 2>/dev/null; then
